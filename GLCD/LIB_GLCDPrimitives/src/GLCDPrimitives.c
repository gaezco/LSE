...
void GLCDPrimitives_ClearScreen(uint8_t color)
{
	...
	{
		for (j = 0; j < (GLCDLowLevel_DISPLAY_HEIGHT/GLCDLowLevel_DISPLAY_PAGE_SIZE); j++)
		{
 ...
 * --------------------------------------------------------------------------------------
 * */
void GLCDPrimitives_SetDot(uint32_t x, uint32_t y, uint8_t color)
{
	if ((y<0)||(y > (GLCDLowLevel_DISPLAY_HEIGHT-1))||(x<0)||( (x>GLCDLowLevel_DISPLAY_WIDTH-1)  ))
		return;
 ...
 * --------------------------------------------------------------------------------------
 * */
void GLCDPrimitives_DrawLine(uint32_t x1, uint8_t y1, uint32_t x2, uint8_t y2, uint8_t color)
{
	uint32_t length, i, y, yAlt, xTmp, yTmp;
	int32_t m;
	//
	// vertical line
	//
 ...
 * --------------------------------------------------------------------------------------
 * */
void GLCDPrimitives_FillRect(uint32_t x, uint8_t y, uint32_t width, uint8_t height, uint8_t color)
{
	uint32_t mask, pageOffset, h, i;
	height++;
