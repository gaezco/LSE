...
uint8_t TouchPanel_Read ( TouchPanel_TEvent* event )
{
	...
	if (ADValue == 0)
	{
		...
		touchPanel_eventPoints[touchPanel_pointsCounter].xCoordinate = (CAD_Lectura(3) - 480) * 319 / ((3650 - 480));
		// Get y coordinate
		PinsConfig_DetectYCoordinate();
		touchPanel_eventPoints[touchPanel_pointsCounter].yCoordinate = (3500 - CAD_Lectura(1)) * 239 / ((3500 - 600)); //  (CAD_Lectura(1) - 600) * 239 / ((3500 - 600));
	}
	else
	{
