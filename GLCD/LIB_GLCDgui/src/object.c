...
void GUI_DrawObject ( TObject* object )
{
	...
	switch (object->objectType)
	{
		...
		case CHECK:
			GUI_Check_Draw ( (TCheck*) object->objectData );
		break;
	}
}
...
uint8_t GUI_Object_RaiseEvent ( TObject* object, GLCDgui_TEvent* event )
{
	...
	switch (object->objectType)
	{
		...
		case CHECK:
					pCheck = (TCheck*) object->objectData;
					if ( (_xCoor > pCheck->xPos) && (_xCoor < pCheck->xPos + pCheck->width) )
					{
						if ( (_yCoor > pCheck->yPos) && (_yCoor < pCheck->yPos + pCheck->height) )
						{
							if (pCheck->OnClickHandler != NULL)
							{
								pCheck->OnClickHandler ( pCheck );
								return 1;
							}
							return 0;
						}
